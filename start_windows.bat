@echo off
echo ===================================================
echo A.R.I.A. - Automated Road Inspection & Accountability
echo Windows Startup Script (uv + FastAPI + Vite)
echo ===================================================
echo.

cd /d "%~dp0"

set "ARIA_API_KEY=test-api-key"

echo [1/5] Stopping existing A.R.I.A. listeners on ports 3000 and 8000...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ports = @(3000, 8000); netstat -ano | ForEach-Object { if ($_ -match '^\s*TCP\s+\S+:(\d+)\s+\S+\s+LISTENING\s+(\d+)\s*$' -and $ports -contains [int]$matches[1]) { $matches[2] } } | Sort-Object -Unique | ForEach-Object { Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue }"
echo.

echo [2/5] Synchronizing dependencies with uv...
call uv sync
if errorlevel 1 (
    echo Error: uv sync failed. Please verify your uv installation and pyproject.toml.
    pause
    exit /b 1
)

echo.
echo [3/5] Checking YOLO Model Weights...
call uv run python -m scripts.download_model

echo.
echo [4/5] Starting FastAPI Backend (Port 8000)...
start "ARIA Backend" cmd /c "set ARIA_API_KEY=%ARIA_API_KEY%&& uv run uvicorn aria.api.app:app --reload --port 8000"

echo.
echo [5/5] Starting Frontend (Port 3000)...
cd frontend
if not exist "node_modules" (
    echo Installing frontend dependencies...
    cmd /c npm install
)
(
    echo VITE_ARIA_API_URL=""
) > .env.local
start "ARIA Frontend" cmd /c "npm run dev -- --strictPort"

echo.
echo ===================================================
echo A.R.I.A. Services have been launched in new windows.
echo Frontend URL: http://localhost:3000
echo Backend API:  http://localhost:8000/docs
echo Tests:        uv run pytest tests -q
echo ===================================================
pause
