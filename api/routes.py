...
_UPLOAD_DIR: str = os.environ.get("ARIA_UPLOAD_DIR", "./data/uploads")
...
def _pipeline_http_exception(pipeline_result: PipelineResult, inspection_id: int) -> HTTPException:
    if pipeline_result.error_code == "INVALID_IMAGE":
        return HTTPException(
            422,
            f"{pipeline_result.failure_reason} Inspection attempt logged under ID {inspection_id}.",
        )
    ...
    return HTTPException(
        503,
        f"{pipeline_result.failure_reason or 'Image could not be processed.'} Inspection attempt logged under ID {inspection_id}.",
    )
...
async def detect(
    ...
    try:
        saved_image_path = _save_uploaded_image(img_bytes, file.content_type or "application/octet-stream")
    except Exception:
